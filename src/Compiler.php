...
//        string $ctag,
    ...
    {
        $code = $this->walk($nodes);
        $key = ucfirst(md5($code));

        if (!isset($this->blocks[$key])) {
            $this->blocks[$key] = sprintf($this->prepare(self::BLOCK_FUNCTION), $key, $code);
        }
...
//        string $indent,
    ...
    {
        $indentNextLine = (str_ends_with($text, "\n"));
     ...
     */
    private function getEscape(): string
    {
        $value = '$value';
